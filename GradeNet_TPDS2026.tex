...
\textbf{3. SLA Violations.} Many interactive jobs (real-time monitoring, online inference) have strict latency SLAs ($<100$ms). But without network-aware scheduling, these SLAs fail 51.2\% of the time (measured in our cluster), wasting computational resources on unusable results.
...
\item \textbf{Grade 1 (medium demand):} Communication volume $2$--$8$ GB. Examples: BERT-base, XGBoost. These have moderate synchronization overhead but can tolerate $50$--$100$ms latency.
...
ALLOCATE_BANDWIDTH(B, J, grades):
  ...
  if util < 0.70:
    ...
    G0 = {i: grades[i]==0}
    bw_G2 = 0.50 * B
    for i in G2: bw[i] = bw_G2 / |G2|
    bw_G1 = 0.30 * B
    for i in G1: bw[i] = bw_G1 / |G1|
    bw_G0 = 0.20 * B
    if |queue| >= 5 or timeout:
      for i in queue:
        bw[i] = bw_G0 / |queue|
\end{verbatim}
\end{small}

Execution time: $\ll 1$ms in SLURM's job admission controller.
